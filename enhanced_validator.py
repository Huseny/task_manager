#!/usr/bin/env python3
"""Unified English validator for task delivery packages.

This validator merges:
- Structural checks from legacy `validator.py`
- Original-session checks from direct-session validator logic
- Aquila cross-checks for metadata prompt/project_type/database
"""

from __future__ import annotations

import argparse
import json
import os
import re
import shutil
import string
import subprocess
import sys
import unicodedata
import urllib.parse
import urllib.request
import zipfile
from dataclasses import dataclass, field
from datetime import datetime, timedelta, timezone
from pathlib import Path
from typing import Any


ALLOWED_PROJECT_TYPES = {"web", "server", "fullstack", "android", "ios", "desktop"}
REQUIRED_METADATA_FIELDS = {
    "prompt",
    "project_type",
    "frontend_language",
    "backend_language",
    "frontend_framework",
    "backend_framework",
    "database",
}
SESSION_PROMPT_ANCHOR_LEN = 50
SESSION_PROMPT_SIMILARITY_THRESHOLD = 0.95
SESSION_PROMPT_CANDIDATE_WINDOW_MINUTES = 20
SESSION_PROMPT_EXEMPT_CONTENT_PREFIXES = (
    "<local-command-caveat>",
    "unknown skill: effot",
)
ORIGINAL_SESSIONS_DIR = "original_sessions"
FORBIDDEN_KEYWORDS = (
    "clean_original_sessions.py",
    "validate_package.py",
    "check_chinese.py",
    "merge_claude_subagents_trajectory.py",
)
DEFAULT_TASK_URL = "https://api.aquila-core.net/api/v1/mindflow-tasks/{task_id}"
DEFAULT_WORKSPACE_DIR = "project_bundle"
DEFAULT_REPO_DIR_NAME = "repo"
DEFAULT_SESSIONS_DIR_NAME = "original_sessions"
TASK_ID_ENV_VARS = ("TASK_ID", "AQUILA_TASK_ID")
ACCESS_TOKEN_ENV_VARS = ("AQUILA_ACCESS_TOKEN", "ACCESS_TOKEN", "TASK_ACCESS_TOKEN")
GITHUB_PAT_ENV_VARS = ("GITHUB_PAT", "GITHUB_TOKEN")


def first_non_empty(values: list[Any]) -> str | None:
    for value in values:
        if isinstance(value, str) and value.strip():
            return value.strip()
    return None


def get_env_value(names: tuple[str, ...]) -> str | None:
    for name in names:
        value = os.getenv(name)
        if value and value.strip():
            return value.strip()
    return None


def sanitize_name(value: str) -> str:
    cleaned = re.sub(r"[^A-Za-z0-9._-]+", "_", value.strip())
    return cleaned.strip("._-") or "task"


def build_task_url(template: str, task_id: str) -> str:
    encoded_task_id = urllib.parse.quote(task_id, safe="")
    if "{task_id}" in template:
        return template.format(task_id=encoded_task_id)
    if "{t_id}" in template:
        return template.format(t_id=encoded_task_id)
    return template.rstrip("/") + f"/{encoded_task_id}"


def http_get_json(url: str, bearer_token: str) -> Any:
    request = urllib.request.Request(
        url,
        headers={
            "Authorization": f"Bearer {bearer_token}",
            "Accept": "application/json",
        },
        method="GET",
    )
    with urllib.request.urlopen(request) as response:
        raw = response.read().decode("utf-8", errors="replace")
    return json.loads(raw)


def extract_task_payload(payload: Any) -> dict[str, Any]:
    if isinstance(payload, dict):
        for key in ("item", "data", "task", "result"):
            candidate = payload.get(key)
            if isinstance(candidate, dict):
                return candidate
        return payload
    raise ValueError("Task payload is not a JSON object")


def extract_task_id(task: dict[str, Any]) -> str:
    return first_non_empty([task.get("mindflow_id"), task.get("task_id"), task.get("id")]) or "unknown_task"


def detect_repo_url(task: dict[str, Any]) -> str:
    repo_url = first_non_empty([task.get("github_link")])
    if not repo_url:
        raise ValueError("github_link is missing from the task payload")
    return repo_url


def detect_sessions_zip_url(task: dict[str, Any]) -> str:
    session_links = task.get("session_files_links")
    if isinstance(session_links, list) and session_links:
        first_link = first_non_empty([session_links[0]])
        if first_link:
            return first_link
    if isinstance(session_links, str):
        value = session_links.strip()
        if value:
            return value
    raise ValueError("session_files_links is missing from the task payload")


def add_pat_to_github_url(repo_url: str, github_pat: str) -> str:
    quoted_pat = urllib.parse.quote(github_pat, safe="")
    if repo_url.startswith("https://"):
        return repo_url.replace("https://", f"https://x-access-token:{quoted_pat}@", 1)
    if repo_url.startswith("http://"):
        return repo_url.replace("http://", f"http://x-access-token:{quoted_pat}@", 1)
    raise ValueError(f"Unsupported repo URL scheme: {repo_url}")


def clone_repo(repo_url: str, github_pat: str | None, target_dir: Path) -> None:
    if target_dir.exists():
        shutil.rmtree(target_dir)
    target_dir.parent.mkdir(parents=True, exist_ok=True)
    clone_url = add_pat_to_github_url(repo_url, github_pat) if github_pat else repo_url
    subprocess.run(["git", "clone", "--depth", "1", clone_url, str(target_dir)], check=True)


def download_file(url: str, out_path: Path) -> None:
    out_path.parent.mkdir(parents=True, exist_ok=True)
    request = urllib.request.Request(url, method="GET")
    with urllib.request.urlopen(request) as response, out_path.open("wb") as handle:
        shutil.copyfileobj(response, handle)


def collapse_wrapper_directory(root: Path) -> Path:
    entries = [entry for entry in root.iterdir() if entry.name != "__MACOSX"]
    if len(entries) == 1 and entries[0].is_dir():
        return entries[0]
    return root


def locate_sessions_source(extracted_root: Path) -> Path:
    for candidate in extracted_root.rglob(DEFAULT_SESSIONS_DIR_NAME):
        if candidate.is_dir():
            return candidate
    for candidate in extracted_root.rglob("original-sessions"):
        if candidate.is_dir():
            return candidate
    return collapse_wrapper_directory(extracted_root)


def copy_directory_contents(source_dir: Path, target_dir: Path) -> None:
    if target_dir.exists():
        shutil.rmtree(target_dir)
    target_dir.mkdir(parents=True, exist_ok=True)
    for entry in source_dir.iterdir():
        destination = target_dir / entry.name
        if entry.is_dir():
            shutil.copytree(entry, destination)
        else:
            shutil.copy2(entry, destination)


def download_and_extract_sessions(sessions_zip_url: str, target_dir: Path, staging_dir: Path) -> Path:
    staging_dir.mkdir(parents=True, exist_ok=True)
    archive_path = staging_dir / "sessions.zip"
    extract_root = staging_dir / "sessions_extract"
    if extract_root.exists():
        shutil.rmtree(extract_root)
    download_file(sessions_zip_url, archive_path)
    with zipfile.ZipFile(archive_path, "r") as archive:
        archive.extractall(extract_root)
    source_root = locate_sessions_source(extracted_root=extract_root)
    copy_directory_contents(source_root, target_dir)
    return target_dir


def normalize_aquila_value(value: Any) -> str:
    if value is None:
        return "none"
    if isinstance(value, list):
        values = [str(v).strip() for v in value if v is not None and str(v).strip()]
        return ", ".join(values) if values else "none"
    if isinstance(value, str):
        out = value.strip()
        return out if out else "none"
    return str(value)


def values_equal(left: Any, right: Any) -> bool:
    return normalize_aquila_value(left) == normalize_aquila_value(right)


def sync_metadata_prompt_with_aquila(repo_dir: Path, aquila_prompt: str | None) -> dict[str, Any]:
    metadata_path = repo_dir / "metadata.json"
    if not metadata_path.is_file():
        return {"performed": False, "reason": "metadata_missing"}
    if not aquila_prompt or not aquila_prompt.strip():
        return {"performed": False, "reason": "no_aquila_prompt"}
    try:
        parsed = json.loads(metadata_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return {"performed": False, "reason": "metadata_unreadable"}
    if not isinstance(parsed, dict):
        return {"performed": False, "reason": "metadata_not_object"}

    existing_text = str(parsed.get("prompt", "")).strip()
    if existing_text == aquila_prompt.strip():
        return {"performed": True, "reason": "already_matches"}

    parsed["prompt"] = aquila_prompt
    metadata_path.write_text(json.dumps(parsed, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    return {"performed": True, "reason": "updated"}


def sync_metadata_project_info_with_aquila(repo_dir: Path, task: dict[str, Any]) -> dict[str, Any]:
    metadata_path = repo_dir / "metadata.json"
    if not metadata_path.is_file():
        return {"performed": False, "reason": "metadata_missing"}
    try:
        parsed = json.loads(metadata_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return {"performed": False, "reason": "metadata_unreadable"}
    if not isinstance(parsed, dict):
        return {"performed": False, "reason": "metadata_not_object"}

    changes: list[str] = []
    field_map = {
        "project_type": "project_type",
        "frontend_framework": "frontend_tech_stack",
        "backend_framework": "backend_stack",
        "database": "database",
    }
    for metadata_field, task_field in field_map.items():
        new_value = normalize_aquila_value(task.get(task_field))
        if not values_equal(parsed.get(metadata_field), new_value):
            parsed[metadata_field] = new_value
            changes.append(metadata_field)

    for language_field in ("frontend_language", "backend_language"):
        if language_field not in parsed or parsed[language_field] in (None, "", []):
            parsed[language_field] = "none"
            changes.append(language_field)

    if normalize_aquila_value(parsed.get("backend_framework")) == "none" and parsed.get("backend_language") not in {None, "none", ""}:
        parsed["backend_language"] = "none"
        changes.append("backend_language")
    if normalize_aquila_value(parsed.get("frontend_framework")) == "none" and parsed.get("frontend_language") not in {None, "none", ""}:
        parsed["frontend_language"] = "none"
        changes.append("frontend_language")

    if changes:
        metadata_path.write_text(json.dumps(parsed, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    return {"performed": True, "fields_changed": sorted(set(changes))}


def prepare_workspace_for_ci(args: argparse.Namespace) -> Path:
    task_id = args.task_id or get_env_value(TASK_ID_ENV_VARS)
    access_token = args.access_token or get_env_value(ACCESS_TOKEN_ENV_VARS)
    github_pat = args.github_pat or get_env_value(GITHUB_PAT_ENV_VARS)

    if not task_id:
        raise SystemExit("TASK_ID is required")
    if not access_token:
        raise SystemExit("AQUILA_ACCESS_TOKEN is required")

    task_url = build_task_url(args.task_url_template, task_id)
    print(f"[{task_id}] fetching task from {task_url}")
    payload = http_get_json(task_url, access_token)
    task = extract_task_payload(payload)

    repo_url = detect_repo_url(task)
    sessions_zip_url = detect_sessions_zip_url(task)
    extracted_task_id = sanitize_name(extract_task_id(task))

    workspace_dir = args.workspace_dir.resolve()
    repo_dir = workspace_dir / args.repo_dir_name
    task_work_dir = workspace_dir / ".task_validation" / extracted_task_id
    sessions_dir = repo_dir / DEFAULT_SESSIONS_DIR_NAME

    if task_work_dir.exists():
        shutil.rmtree(task_work_dir)
    task_work_dir.mkdir(parents=True, exist_ok=True)

    if repo_dir.exists():
        shutil.rmtree(repo_dir)

    print(f"[{task_id}] cloning repository: {repo_url}")
    clone_repo(repo_url, github_pat, repo_dir)
    prompt_sync = sync_metadata_prompt_with_aquila(repo_dir, task.get("prompt_text") if isinstance(task.get("prompt_text"), str) else None)
    print(f"[{task_id}] metadata prompt sync: {prompt_sync}")
    project_sync = sync_metadata_project_info_with_aquila(repo_dir, task)
    print(f"[{task_id}] metadata project sync: {project_sync}")

    download_and_extract_sessions(sessions_zip_url, sessions_dir, task_work_dir)
    print(f"[{task_id}] sessions extracted to {sessions_dir}")

    args.build_dir_file.parent.mkdir(parents=True, exist_ok=True)
    args.build_dir_file.write_text(str(repo_dir.resolve()) + "\n", encoding="utf-8")
    print(f"[{task_id}] build directory written to {args.build_dir_file}")
    return repo_dir


@dataclass
class CheckItem:
    status: str
    message: str
    rel_path: str


@dataclass
class CheckSection:
    title: str
    items: list[CheckItem] = field(default_factory=list)

    def add_pass(self, message: str, rel_path: str = ".") -> None:
        self.items.append(CheckItem("PASS", message, rel_path))

    def add_fail(self, message: str, rel_path: str = ".") -> None:
        self.items.append(CheckItem("FAIL", message, rel_path))

    def add_warn(self, message: str, rel_path: str = ".") -> None:
        self.items.append(CheckItem("WARN", message, rel_path))


@dataclass
class JsonlAnalysis:
    readable: bool
    first_timestamp_raw: str | None = None
    first_timestamp_line: int | None = None
    latest_timestamp_dt: datetime | None = None
    latest_timestamp_raw: str | None = None
    latest_timestamp_line: int | None = None
    last_semantic_line: int | None = None
    last_semantic_role: str | None = None
    last_semantic_has_assistant_text: bool = False
    user_line_count: int = 0
    exempt_user_line_count: int = 0
    user_candidates: list[tuple[str, int, str, datetime]] = field(default_factory=list)
    keyword_line_numbers: dict[str, list[int]] = field(default_factory=dict)


class UnifiedValidator:
    def __init__(self, target: str, task_id: str | None, access_token: str | None, task_url_template: str) -> None:
        self.target = target
        self.task_id = task_id
        self.access_token = access_token
        self.task_url_template = task_url_template
        self.root: Path | None = None
        self.sections: list[CheckSection] = []
        self.metadata: dict[str, Any] = {}

    def run(self) -> int:
        self._check_input_root()
        if self.root is None:
            self._write_report(Path.cwd() / ".tmp" / "validation_report.md")
            return 1

        self._check_structure()
        self._check_metadata_schema()
        self._check_original_sessions()
        self._check_aquila_alignment()

        report_path = self.root / ".tmp" / "validation_report.md"
        self._write_report(report_path)
        fails = self._fail_count()
        print(f"{'PASS' if fails == 0 else 'FAIL'} | errors={fails} | report={report_path}")
        return 0 if fails == 0 else 1

    def _new_section(self, title: str) -> CheckSection:
        section = CheckSection(title=title)
        self.sections.append(section)
        return section

    def _rel(self, path: Path | str) -> str:
        if isinstance(path, str):
            return path
        if self.root is None:
            return path.as_posix()
        try:
            rel = path.relative_to(self.root).as_posix()
            return rel if rel else "."
        except ValueError:
            return path.as_posix()

    def _fail_count(self) -> int:
        return sum(1 for section in self.sections for item in section.items if item.status == "FAIL")

    def _check_input_root(self) -> None:
        section = self._new_section("1. Input Directory")
        candidate = Path(self.target).expanduser()
        if not candidate.is_dir():
            if not candidate.is_absolute() and (Path.cwd() / candidate).is_dir():
                candidate = (Path.cwd() / candidate)
            else:
                section.add_fail(f"Input directory does not exist: {self.target}", self.target)
                return
        self.root = candidate.resolve()
        section.add_pass("Input directory is valid", self._rel(self.root))

    def _read_text(self, path: Path) -> str | None:
        if not path.is_file():
            return None
        try:
            data = path.read_bytes()
        except OSError:
            return None
        if b"\x00" in data:
            return None
        for enc in ("utf-8", "utf-8-sig", "gb18030"):
            try:
                return data.decode(enc)
            except UnicodeDecodeError:
                continue
        return None

    def _is_temp_entry(self, path: Path) -> bool:
        lower_name = path.name.lower()
        temp_exact_names = {
            "tmp",
            "temp",
            "temporary",
            ".temp",
            "__pycache__",
            ".pytest_cache",
            ".mypy_cache",
            ".ruff_cache",
            ".cache",
        }
        temp_suffixes = {".temp", ".swp", ".swo", ".bak", ".old", ".orig", ".rej"}
        if lower_name in temp_exact_names:
            return True
        if lower_name.endswith("~") or lower_name.startswith("~$"):
            return True
        if any(lower_name.endswith(suffix) for suffix in temp_suffixes):
            return True
        if lower_name.startswith("tmp-") or lower_name.startswith("temp-"):
            return True
        if lower_name.endswith("-tmp") or lower_name.endswith("-temp"):
            return True
        return False

    def _check_structure(self) -> None:
        assert self.root is not None
        section = self._new_section("2. Structure")

        mandatory_dirs = ["docs", "repo", ".tmp", ORIGINAL_SESSIONS_DIR]
        mandatory_files = [
            "docs/design.md",
            "docs/questions.md",
            "metadata.json",
            "repo/docker-compose.yml",
            "repo/README.md",
        ]

        for dirname in mandatory_dirs:
            path = self.root / dirname
            if path.is_dir():
                section.add_pass(f"Directory exists: {dirname}/", self._rel(path))
            else:
                section.add_fail(f"Missing directory: {dirname}/", self._rel(path))

        for file_name in mandatory_files:
            path = self.root / file_name
            if path.is_file():
                section.add_pass(f"File exists: {file_name}", self._rel(path))
            else:
                section.add_fail(f"Missing file: {file_name}", self._rel(path))

        allowed_root_entries = {
            "docs",
            "repo",
            ".tmp",
            "metadata.json",
            ORIGINAL_SESSIONS_DIR,
            ".git",
            ".gitignore",
            ".github",
        }
        for entry in sorted(self.root.iterdir(), key=lambda p: p.name.lower()):
            if entry.name not in allowed_root_entries and entry.name != "validation_report.md":
                section.add_fail(
                    f"Unexpected {'directory' if entry.is_dir() else 'file'} in root: {entry.name}",
                    self._rel(entry),
                )

        docs_dir = self.root / "docs"
        if docs_dir.is_dir():
            for entry in sorted(docs_dir.iterdir(), key=lambda p: p.name.lower()):
                if entry.is_dir() or entry.suffix.lower() != ".md":
                    section.add_fail(
                        f"Invalid {'directory' if entry.is_dir() else 'file'} in docs/: {entry.name}",
                        self._rel(entry),
                    )

        self._check_tmp_layout(section)

        test_options = ["repo/run_test.sh", "repo/run_tests.sh"]
        found_test = next((opt for opt in test_options if (self.root / opt).is_file()), None)
        if found_test:
            section.add_pass(f"Test script found: {found_test}", found_test)
        else:
            section.add_fail("Missing test script (run_test.sh or run_tests.sh)", "repo/")

        for entry in self.root.rglob("*"):
            if self._is_temp_entry(entry):
                section.add_fail(
                    f"Temporary {'directory' if entry.is_dir() else 'file'} found: {entry.relative_to(self.root)}",
                    self._rel(entry),
                )

    def _check_tmp_layout(self, section: CheckSection) -> None:
        assert self.root is not None
        tmp_dir = self.root / ".tmp"
        if not tmp_dir.is_dir():
            return

        tmp_entries = list(tmp_dir.iterdir())
        tmp_files = [e for e in tmp_entries if e.is_file()]
        tmp_dirs = [e for e in tmp_entries if e.is_dir()]
        required_extra_file = "test_coverage_and_readme_audit_report.md"

        for entry in tmp_dirs:
            section.add_fail(f"Invalid directory in .tmp/: {entry.name}", self._rel(entry))

        audit_re = re.compile(r"^audit_report-(\d+)\.md$")
        fix_re = re.compile(r"^audit_report-(\d+)-fix_check\.md$")
        audit_nums: list[str] = []
        fix_nums: list[str] = []
        report_files: list[str] = []
        required_file_count = 0

        for file in tmp_files:
            name = file.name
            m_audit = audit_re.match(name)
            m_fix = fix_re.match(name)
            if m_fix:
                fix_nums.append(m_fix.group(1))
                report_files.append(name)
            elif m_audit:
                audit_nums.append(m_audit.group(1))
                report_files.append(name)
            elif name == required_extra_file:
                required_file_count += 1
            else:
                section.add_fail(f"Invalid file in .tmp/: {name}", self._rel(file))

        if required_file_count != 1:
            section.add_fail(
                "Missing or duplicate test_coverage_and_readme_audit_report.md in .tmp/",
                self._rel(tmp_dir),
            )
        if len(report_files) != 4:
            section.add_fail(
                f".tmp must contain exactly 4 audit/fix-check files, found {len(report_files)}",
                self._rel(tmp_dir),
            )
        if len(tmp_files) != 5:
            section.add_fail(
                f".tmp must contain exactly 5 files, found {len(tmp_files)}",
                self._rel(tmp_dir),
            )
        if len(fix_nums) not in {1, 2}:
            section.add_fail(
                f"Expected 1 or 2 fix-check reports, found {len(fix_nums)}",
                self._rel(tmp_dir),
            )
        elif len(fix_nums) == 2 and (len(audit_nums) != 2 or sorted(audit_nums) != sorted(fix_nums)):
            section.add_fail(
                "When there are 2 fix-check reports, audit and fix-check numbers must match",
                self._rel(tmp_dir),
            )
        elif len(fix_nums) == 1:
            if len(audit_nums) != 3 or fix_nums[0] != "1" or "3" not in audit_nums:
                section.add_fail(
                    "With one fix-check report, expected audit_report-1-fix_check.md and 3 audits including audit_report-3.md",
                    self._rel(tmp_dir),
                )

    def _check_metadata_schema(self) -> None:
        assert self.root is not None
        section = self._new_section("3. metadata.json")
        path = self.root / "metadata.json"
        content = self._read_text(path)
        if content is None:
            section.add_fail("metadata.json is missing or not readable", self._rel(path))
            return

        try:
            parsed = json.loads(content)
        except json.JSONDecodeError as exc:
            section.add_fail(f"metadata.json is invalid JSON: {exc.msg}", self._rel(path))
            return

        if not isinstance(parsed, dict):
            section.add_fail("metadata.json root must be a JSON object", self._rel(path))
            return

        self.metadata = parsed
        missing = sorted(REQUIRED_METADATA_FIELDS - set(parsed.keys()))
        extra = sorted(set(parsed.keys()) - REQUIRED_METADATA_FIELDS)
        if missing:
            section.add_fail(f"Missing required metadata fields: {', '.join(missing)}", self._rel(path))
        if extra:
            section.add_fail(f"Unexpected metadata fields: {', '.join(extra)}", self._rel(path))

        for key in REQUIRED_METADATA_FIELDS:
            if key not in parsed:
                continue
            value = parsed.get(key)
            if not isinstance(value, str):
                section.add_fail(f"metadata.{key} must be a string", self._rel(path))
                continue
            if key not in {"backend_language", "backend_framework", "frontend_language", "frontend_framework"} and not value.strip():
                section.add_fail(f"metadata.{key} cannot be empty", self._rel(path))

        project_type = str(parsed.get("project_type", "")).strip().lower()
        if project_type and project_type not in ALLOWED_PROJECT_TYPES:
            section.add_fail(
                f"metadata.project_type must be one of: {', '.join(sorted(ALLOWED_PROJECT_TYPES))}",
                self._rel(path),
            )

    def _normalize_for_compare(self, text: str) -> str:
        text = unicodedata.normalize("NFKC", text or "")
        text = text.replace(r"\n", "").replace(r"\r", "").replace(r"\t", "")
        text = text.replace(r'\"', "").replace(r"\'", "")
        text = re.sub(r"[^A-Za-z0-9\u4e00-\u9fff]+", "", text)
        return text

    def _strip_trailing_punct(self, text: str | None) -> str:
        if text is None:
            return ""
        out = text.rstrip()
        trailing = string.punctuation + "，。！？；：、“”‘’（）【】《》—…"
        while out and out[-1] in trailing:
            out = out[:-1].rstrip()
        return out

    def _compare_by_anchor(self, reference_text: str, target_text: str) -> dict[str, Any]:
        head = reference_text.lstrip()[:SESSION_PROMPT_ANCHOR_LEN]
        tail = self._strip_trailing_punct(reference_text)[-SESSION_PROMPT_ANCHOR_LEN:]
        if not head and not tail:
            return {"near_duplicate": False, "similarity": 0.0, "matched": False}
        if head and tail:
            pattern = rf"{re.escape(head)}(.*?){re.escape(tail)}"
        elif head:
            pattern = rf"{re.escape(head)}(.*)"
        else:
            pattern = rf"(.*?){re.escape(tail)}"
        match = re.search(pattern, target_text, flags=re.DOTALL)
        candidate = match.group(0) if match else target_text
        s1 = self._normalize_for_compare(reference_text)
        s2 = self._normalize_for_compare(candidate)
        similarity = 1.0 if s1 == s2 and s1 else __import__("difflib").SequenceMatcher(None, s1, s2).ratio()
        return {
            "near_duplicate": similarity >= SESSION_PROMPT_SIMILARITY_THRESHOLD,
            "similarity": similarity,
            "matched": bool(match),
        }

    def _parse_ts(self, value: str | None) -> datetime | None:
        if not value:
            return None
        raw = value.strip()
        if not raw:
            return None
        if raw.endswith("Z"):
            raw = raw[:-1] + "+00:00"
        try:
            dt = datetime.fromisoformat(raw)
        except ValueError:
            return None
        if dt.tzinfo is None:
            return dt.replace(tzinfo=timezone.utc)
        return dt.astimezone(timezone.utc)

    def _extract_text_content(self, message: dict[str, Any]) -> str:
        content = message.get("content")
        if isinstance(content, str):
            return content
        if isinstance(content, list):
            out: list[str] = []
            for item in content:
                if isinstance(item, str):
                    out.append(item)
                elif isinstance(item, dict) and str(item.get("type", "")).lower() == "text":
                    out.append(str(item.get("text", "")))
            return "\n".join(out)
        if isinstance(content, dict) and str(content.get("type", "")).lower() == "text":
            return str(content.get("text", ""))
        return ""

    def _is_local_command_noise(self, payload: dict[str, Any]) -> bool:
        message = payload.get("message")
        if not isinstance(message, dict):
            return False
        if str(message.get("role", "")).lower() != "user":
            return False
        text = self._extract_text_content(message).strip()
        return text.startswith(
            (
                "<local-command-caveat>",
                "<command-name>",
                "<command-message>",
                "<command-args>",
                "<local-command-stdout>",
                "<local-command-stderr>",
            )
        )

    def _analyze_jsonl(self, path: Path) -> JsonlAnalysis:
        text = self._read_text(path)
        analysis = JsonlAnalysis(
            readable=text is not None,
            keyword_line_numbers={keyword: [] for keyword in FORBIDDEN_KEYWORDS},
        )
        if text is None:
            return analysis

        for line_no, raw in enumerate(text.splitlines(), start=1):
            lower = raw.lower()
            for keyword in FORBIDDEN_KEYWORDS:
                if keyword in lower:
                    analysis.keyword_line_numbers[keyword].append(line_no)

            line = raw.strip()
            if not line:
                continue
            try:
                payload = json.loads(line)
            except json.JSONDecodeError:
                continue
            if not isinstance(payload, dict):
                continue

            timestamp_raw = payload.get("timestamp") if isinstance(payload.get("timestamp"), str) else None
            parsed_ts = self._parse_ts(timestamp_raw)
            if timestamp_raw and analysis.first_timestamp_raw is None:
                analysis.first_timestamp_raw = timestamp_raw
                analysis.first_timestamp_line = line_no
            if parsed_ts is not None and (analysis.latest_timestamp_dt is None or parsed_ts > analysis.latest_timestamp_dt):
                analysis.latest_timestamp_dt = parsed_ts
                analysis.latest_timestamp_raw = timestamp_raw
                analysis.latest_timestamp_line = line_no

            message = payload.get("message")
            if isinstance(message, dict):
                role = str(message.get("role", "")).strip().lower()
                msg_type = str(message.get("type", "")).strip().lower()
                if (not msg_type or msg_type == "message") and not self._is_local_command_noise(payload):
                    analysis.last_semantic_line = line_no
                    analysis.last_semantic_role = role
                    analysis.last_semantic_has_assistant_text = role == "assistant" and bool(self._extract_text_content(message).strip())

                if role == "user":
                    analysis.user_line_count += 1
                    content_text = self._extract_text_content(message)
                    if not content_text.strip():
                        continue
                    if self._is_local_command_noise(payload) or content_text.lstrip().lower().startswith(SESSION_PROMPT_EXEMPT_CONTENT_PREFIXES):
                        analysis.exempt_user_line_count += 1
                        continue
                    if timestamp_raw and parsed_ts is not None:
                        analysis.user_candidates.append((content_text, line_no, timestamp_raw, parsed_ts))

        return analysis

    def _check_original_sessions(self) -> None:
        assert self.root is not None
        section = self._new_section("4. original_sessions")
        sessions_dir = self.root / ORIGINAL_SESSIONS_DIR
        if not sessions_dir.is_dir():
            section.add_fail("Missing original_sessions/ directory", self._rel(sessions_dir))
            return

        root_legacy = sorted(
            [
                p for p in self.root.iterdir() if p.is_file() and (
                    p.name.lower() == "trajectory.json"
                    or re.fullmatch(r"trajectory[-_]\d+\.json", p.name.lower())
                    or re.fullmatch(r"(trajectory(?:[-_]\d+)?|develop(?:[-_]\d+)?|bugfix(?:[-_]\d+)?)\.json", p.name.lower())
                )
            ],
            key=lambda p: p.name.lower(),
        )
        for p in root_legacy:
            section.add_fail("Legacy session JSON found in root; move it to original_sessions/", self._rel(p))

        entries = list(sessions_dir.iterdir())
        if not entries:
            section.add_fail("original_sessions/ is empty", self._rel(sessions_dir))
            return

        self._check_leading_bundle_structure(section, sessions_dir)
        self._check_session_prompt_anchor(section, sessions_dir)
        self._check_latest_trajectory(section, sessions_dir)
        self._check_forbidden_keywords(section, sessions_dir)

    def _check_leading_bundle_structure(self, section: CheckSection, sessions_dir: Path) -> None:
        child_dirs = sorted([p for p in sessions_dir.iterdir() if p.is_dir()], key=lambda p: p.name.lower())
        exempt = {"tool-results", "memory"}
        session_dirs = [p for p in child_dirs if p.name.lower() not in exempt]

        if not session_dirs:
            section.add_warn("No session_id directories were found under original_sessions/", self._rel(sessions_dir))
            return

        failures = 0
        for session_dir in session_dirs:
            session_id = session_dir.name
            required_jsonl = sessions_dir / f"{session_id}.jsonl"
            if not required_jsonl.is_file():
                failures += 1
                section.add_fail(
                    f"Missing required top-level session file {session_id}.jsonl for directory {session_id}/",
                    self._rel(session_dir),
                )
            subagents_dir = session_dir / "subagents"
            if not subagents_dir.is_dir():
                continue
            for jsonl_file in sorted(subagents_dir.glob("*.jsonl"), key=lambda p: p.name.lower()):
                stem = jsonl_file.stem.lower()
                if not stem.startswith("agent-"):
                    continue
                if stem.startswith("agent-aside_question") or "acompact" in stem:
                    continue
                meta = subagents_dir / f"{jsonl_file.stem}.meta.json"
                if not meta.is_file():
                    failures += 1
                    section.add_fail(f"Missing meta file {meta.name} for {jsonl_file.name}", self._rel(jsonl_file))

        if failures == 0:
            section.add_pass("Session directory structure check passed", self._rel(sessions_dir))

    def _collect_first_layer_jsonl(self, sessions_dir: Path) -> list[Path]:
        return sorted([p for p in sessions_dir.iterdir() if p.is_file() and p.suffix.lower() == ".jsonl"], key=lambda p: p.name.lower())

    def _check_latest_trajectory(self, section: CheckSection, sessions_dir: Path) -> None:
        candidates: list[tuple[datetime, str, int, Path]] = []
        for jsonl_path in self._collect_first_layer_jsonl(sessions_dir):
            if jsonl_path.name.lower() == "memory.jsonl":
                continue
            analysis = self._analyze_jsonl(jsonl_path)
            if analysis.latest_timestamp_dt and analysis.latest_timestamp_raw and analysis.latest_timestamp_line:
                candidates.append((analysis.latest_timestamp_dt, analysis.latest_timestamp_raw, analysis.latest_timestamp_line, jsonl_path))

        if not candidates:
            section.add_warn("Skipped latest trajectory completeness check: no usable timestamp in first-layer jsonl files", self._rel(sessions_dir))
            return

        candidates.sort(key=lambda item: (item[0], item[3].as_posix().lower()), reverse=True)
        _, latest_raw, latest_line_no, latest_jsonl = candidates[0]
        analysis = self._analyze_jsonl(latest_jsonl)

        if analysis.last_semantic_line is None:
            section.add_fail("No semantic message event found in latest trajectory file", self._rel(latest_jsonl))
            return
        if (analysis.last_semantic_role or "") != "assistant":
            section.add_fail(
                f"Last semantic message role is not assistant (line {analysis.last_semantic_line}, role={analysis.last_semantic_role or 'unknown'})",
                self._rel(latest_jsonl),
            )
            return
        if not analysis.last_semantic_has_assistant_text:
            section.add_fail(
                f"Last semantic assistant message has no text content (line {analysis.last_semantic_line})",
                self._rel(latest_jsonl),
            )
            return
        section.add_pass(
            f"Latest trajectory completeness passed (file={latest_jsonl.name}, latest_timestamp={latest_raw}, line={latest_line_no})",
            self._rel(latest_jsonl),
        )

    def _check_session_prompt_anchor(self, section: CheckSection, sessions_dir: Path) -> None:
        assert self.root is not None
        prompt_text = str(self.metadata.get("prompt", "")).strip()
        if not prompt_text:
            section.add_warn("Skipped session prompt-anchor check: metadata.prompt is missing or empty", self._rel(self.root / "metadata.json"))
            return

        timestamp_candidates: list[tuple[tuple[int, str], Path, str, int]] = []
        for path in self._collect_first_layer_jsonl(sessions_dir):
            analysis = self._analyze_jsonl(path)
            if analysis.first_timestamp_raw is None or analysis.first_timestamp_line is None:
                continue
            ts = self._parse_ts(analysis.first_timestamp_raw)
            order_key = (0, ts.isoformat()) if ts else (1, analysis.first_timestamp_raw)
            timestamp_candidates.append((order_key, path, analysis.first_timestamp_raw, analysis.first_timestamp_line))

        if not timestamp_candidates:
            section.add_warn("Skipped session prompt-anchor check: no usable timestamp in first-layer jsonl files", self._rel(sessions_dir))
            return

        timestamp_candidates.sort(key=lambda item: (item[0], item[1].as_posix().lower()))
        for _, selected_jsonl, first_ts, first_line in timestamp_candidates:
            analysis = self._analyze_jsonl(selected_jsonl)
            candidates: list[tuple[str, int, str]] = []
            window_start: datetime | None = None
            window_end: datetime | None = None
            for content_text, line_no, ts_raw, parsed_ts in analysis.user_candidates:
                if window_start is None:
                    window_start = parsed_ts
                    window_end = parsed_ts + timedelta(minutes=SESSION_PROMPT_CANDIDATE_WINDOW_MINUTES)
                    candidates.append((content_text, line_no, ts_raw))
                    continue
                if window_end is not None and window_start <= parsed_ts <= window_end:
                    candidates.append((content_text, line_no, ts_raw))

            if not candidates:
                continue

            best_similarity = -1.0
            best_line = -1
            for content_text, line_no, _ in candidates:
                compare = self._compare_by_anchor(prompt_text, content_text)
                similarity = float(compare["similarity"])
                if similarity > best_similarity:
                    best_similarity = similarity
                    best_line = line_no
                if bool(compare["near_duplicate"]):
                    section.add_pass(
                        (
                            "Session prompt-anchor check passed "
                            f"(file={selected_jsonl.name}, file_first_timestamp={first_ts}, file_first_timestamp_line={first_line}, "
                            f"match_line={line_no}, similarity={similarity:.6f})"
                        ),
                        self._rel(selected_jsonl),
                    )
                    return

            section.add_fail(
                (
                    "Session prompt-anchor check failed due to low similarity "
                    f"(file={selected_jsonl.name}, file_first_timestamp={first_ts}, best_line={best_line}, "
                    f"best_similarity={best_similarity:.6f}, threshold={SESSION_PROMPT_SIMILARITY_THRESHOLD:.6f})"
                ),
                self._rel(selected_jsonl),
            )
            return

        section.add_warn("Skipped session prompt-anchor check: no comparable user content found", self._rel(sessions_dir))

    def _format_line_numbers(self, line_numbers: list[int]) -> str:
        if not line_numbers:
            return ""
        if len(line_numbers) <= 20:
            return ", ".join(str(n) for n in line_numbers)
        return ", ".join(str(n) for n in line_numbers[:20]) + f" ... total {len(line_numbers)} lines"

    def _check_forbidden_keywords(self, section: CheckSection, sessions_dir: Path) -> None:
        jsonl_files = sorted(sessions_dir.rglob("*.jsonl"), key=lambda p: p.as_posix().lower())
        if not jsonl_files:
            section.add_warn("No jsonl files found under original_sessions; forbidden keyword check skipped", self._rel(sessions_dir))
            return

        findings = 0
        for path in jsonl_files:
            analysis = self._analyze_jsonl(path)
            if not analysis.readable:
                section.add_warn("jsonl file is unreadable; skipped forbidden keyword check", self._rel(path))
                continue
            for keyword in FORBIDDEN_KEYWORDS:
                lines = analysis.keyword_line_numbers.get(keyword, [])
                if not lines:
                    continue
                findings += 1
                section.add_fail(
                    f'Forbidden keyword "{keyword}" detected (lines: {self._format_line_numbers(lines)})',
                    self._rel(path),
                )
        if findings == 0:
            section.add_pass("No forbidden keywords detected in original_sessions/*.jsonl", self._rel(sessions_dir))

    def _normalize_aquila_value(self, value: Any) -> str:
        if value is None:
            return "none"
        if isinstance(value, list):
            values = [str(v).strip() for v in value if v is not None and str(v).strip()]
            return ", ".join(values) if values else "none"
        if isinstance(value, str):
            out = value.strip()
            return out if out else "none"
        return str(value)

    def _fetch_aquila_task(self) -> tuple[dict[str, Any] | None, str | None]:
        task_id = self.task_id or os.getenv("TASK_ID") or os.getenv("AQUILA_TASK_ID")
        token = self.access_token or os.getenv("AQUILA_ACCESS_TOKEN") or os.getenv("ACCESS_TOKEN") or os.getenv("TASK_ACCESS_TOKEN")
        if not task_id or not token:
            return None, "TASK_ID and AQUILA access token are not available"

        encoded_task_id = urllib.parse.quote(task_id, safe="")
        template = self.task_url_template
        if "{task_id}" in template:
            url = template.format(task_id=encoded_task_id)
        elif "{t_id}" in template:
            url = template.format(t_id=encoded_task_id)
        else:
            url = template.rstrip("/") + f"/{encoded_task_id}"

        req = urllib.request.Request(
            url,
            headers={"Authorization": f"Bearer {token}", "Accept": "application/json"},
            method="GET",
        )
        try:
            with urllib.request.urlopen(req) as resp:
                payload = json.loads(resp.read().decode("utf-8", errors="replace"))
        except Exception as exc:  # noqa: BLE001
            return None, f"Failed to fetch Aquila task: {exc}"

        if isinstance(payload, dict):
            for key in ("item", "data", "task", "result"):
                candidate = payload.get(key)
                if isinstance(candidate, dict):
                    return candidate, None
            return payload, None
        return None, "Aquila response is not a JSON object"

    def _check_aquila_alignment(self) -> None:
        section = self._new_section("5. Aquila Alignment")
        task, error = self._fetch_aquila_task()
        if task is None:
            section.add_warn(f"Aquila cross-check skipped: {error}", ".")
            return

        metadata_prompt = str(self.metadata.get("prompt", "")).strip()
        aquila_prompt = str(task.get("prompt_text", "")).strip()
        if not metadata_prompt or not aquila_prompt:
            section.add_warn("Prompt cross-check skipped: metadata.prompt or aquila.prompt_text is empty", "metadata.json")
        else:
            prompt_compare = self._compare_by_anchor(aquila_prompt, metadata_prompt)
            if bool(prompt_compare["near_duplicate"]):
                section.add_pass(
                    f"metadata.prompt matches Aquila prompt_text (similarity={float(prompt_compare['similarity']):.6f})",
                    "metadata.json",
                )
            else:
                section.add_fail(
                    (
                        "metadata.prompt does not match Aquila prompt_text "
                        f"(similarity={float(prompt_compare['similarity']):.6f}, threshold={SESSION_PROMPT_SIMILARITY_THRESHOLD:.6f})"
                    ),
                    "metadata.json",
                )

        metadata_project_type = str(self.metadata.get("project_type", "")).strip().lower()
        aquila_project_type = self._normalize_aquila_value(task.get("project_type")).lower()
        if metadata_project_type == aquila_project_type:
            section.add_pass(
                f"metadata.project_type matches Aquila project_type ({metadata_project_type})",
                "metadata.json",
            )
        else:
            section.add_fail(
                f"metadata.project_type mismatch: metadata={metadata_project_type or 'empty'}, aquila={aquila_project_type}",
                "metadata.json",
            )

        metadata_database = self._normalize_aquila_value(self.metadata.get("database")).lower()
        aquila_database = self._normalize_aquila_value(task.get("database")).lower()
        if metadata_database == aquila_database:
            section.add_pass(
                f"metadata.database matches Aquila database ({metadata_database})",
                "metadata.json",
            )
        else:
            section.add_fail(
                f"metadata.database mismatch: metadata={metadata_database}, aquila={aquila_database}",
                "metadata.json",
            )

    def _write_report(self, report_path: Path) -> None:
        lines = ["# Validation Report", ""]
        for section in self.sections:
            lines.append(f"## {section.title}")
            if not section.items:
                lines.append("- [PASS] No checks")
            else:
                for item in section.items:
                    lines.append(f"- [{item.status}] {item.message} ({item.rel_path})")
            lines.append("")

        content = "\n".join(lines).rstrip() + "\n"
        report_path.parent.mkdir(parents=True, exist_ok=True)
        report_path.write_text(content, encoding="utf-8")


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Unified validator for task delivery packages")
    parser.add_argument("target", nargs="?", default=".", help="Target directory path")
    parser.add_argument("--task-id", help="Aquila task id (optional, can come from env)")
    parser.add_argument("--access-token", help="Aquila bearer token (optional, can come from env)")
    parser.add_argument(
        "--task-url-template",
        default=os.getenv("AQUILA_TASK_API_URL", DEFAULT_TASK_URL),
        help="Aquila task URL template",
    )
    parser.add_argument(
        "--prepare-ci",
        action="store_true",
        help="Fetch task from Aquila, clone repo, and stage original_sessions before validation",
    )
    parser.add_argument(
        "--workspace-dir",
        type=Path,
        default=Path(os.getenv("CLONE_DIR", DEFAULT_WORKSPACE_DIR)),
        help="Workspace directory for prepared task repo",
    )
    parser.add_argument(
        "--repo-dir-name",
        default=DEFAULT_REPO_DIR_NAME,
        help="Repository directory name under workspace dir",
    )
    parser.add_argument(
        "--build-dir-file",
        type=Path,
        default=Path("/tmp/build_dir"),
        help="Path to write prepared repo directory for build steps",
    )
    parser.add_argument(
        "--github-pat",
        help="Optional GitHub PAT for private repositories",
    )
    return parser.parse_args(argv)


def main(argv: list[str]) -> int:
    args = parse_args(argv)
    target = args.target
    if args.prepare_ci:
        prepared_repo_dir = prepare_workspace_for_ci(args)
        target = str(prepared_repo_dir)

    validator = UnifiedValidator(
        target=target,
        task_id=args.task_id,
        access_token=args.access_token,
        task_url_template=args.task_url_template,
    )
    return validator.run()


if __name__ == "__main__":
    sys.exit(main(sys.argv[1:]))
